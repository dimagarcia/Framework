...
//public class Framework {
    ...
    public static void main(String[] args) {
        ...
        */
        ...
        // Obtención de datos de la mezcla gausiana desde la base de datos Oracle (dmixture)
        /*
          X~dnorm(2.0, 1.0)
          Y~dnorm(-2.0, 3.0)
          Z~dbin(0.3,1)
          P<-X*(1-Z) + Y*Z        
        */
        // 2015-MAY-27 Utilización bibliotecas fMEF del MIT
//        int n=2;
//        int n=1;
        int dim=2;
...
//        mh.metropolis();                
        ...
        //**********************************************************************
        mhBN = new HastingBN();        
        jMEF.PVectorMatrix m = new jMEF.PVectorMatrix(2);
        // Mu and sigma R2
        m.v.array[0] = 0;
        m.v.array[1] = 0;     
        // co-varianza
        ...
        m.M.array[1][0] = 0.0;
        m.M.array[1][1] = 1;        
        mhBN.setProposal(new jMEF.MixtureModel(1), new jMEF.MultivariateGaussian(), m);
        mhBN.t = 5000;
        mhBN.x = new jMEF.PVector[mhBN.t];
        mhBN.stationary = h;
//        mhBN.stationary.visible = true;
//x[0].array[0] = -0.7999976034479769, x[0].array[1] = 0.5700480971593643
//xp[0].array[0] = -0.981817874578395, xp[0].array[1] = 2.2827573278763733 
        jMEF.PVector x = new jMEF.PVector(2);
        x.array[0] = -0.7999976034479769;
        x.array[1] = 0.5700480971593643;
        jMEF.PVector xp = new jMEF.PVector(2);
        xp.array[0] = -0.981817874578395;
        xp.array[1] = 2.2827573278763733;
//        double px  = mhBN.stationary.getDensity(x); 
//        double pxp = mhBN.stationary.getDensity(xp);
        double px  = h.getDensity(x); 
        double pxp = h.getDensity(xp);
        System.out.println ("px = " + px + ", pxp = " + pxp + ", alpha = " + pxp/px);
        // 1. Inicializacion Xo
        mhBN.x = mhBN.q.drawRandomPoints(1);                            
        // 2. Iteracion
        //mhBN.metropolis();
/******************************************************************************/   
        jMEF.MixtureModel q = new jMEF.MixtureModel( 2 );
        q.EF = new jMEF.MultivariateGaussian();
        q.weight[0] = 1;
        q.param[0] = m;

        xR2 = new jMEF.PVector[t];        
//        double sigma2 = 0.1; // varianza
//        x[0] = 0.4; // 1. Inicializacion Xo
        jMEF.PVector[] QxR2 = q.drawRandomPoints(1);                    
        xR2[0] = QxR2[0]; // Se invierten ejes X <-> Z
        
        for (int s=0; s < t-1; s++){ // 2. Iteracion
//            double X = x[s];  // 3. Define X = Xs
            jMEF.PVector XR2 = xR2[s];
//            double Xp = dnorm(X,sigma2); // 4. Sample Xp from proposal distribution
            m.v = XR2;
            q.param[0] = m;            
            QxR2 = q.drawRandomPoints(1);
            jMEF.PVector XpR2  = QxR2[0];  // Se invierten ejes X <-> Z 
//            XpR2.array[1] = QxR2[0].array[0];
//            XpR2.array[0] = QxR2[0].array[1];
            
            
            // 5. Compute acceptance probability (alpha)
//            double Px  = (float)f.density(points[i]); dcat(X);
            double Px  = h.getDensity(XR2);
//            double Pxp = dcat(Xp);
            double Pxp = h.getDensity(XpR2);
            double alpha = Pxp / Px; 
...
//                x[s+1] = (Math.random() < alpha)? Xp : x[s];
                xR2[s+1] = (Math.random() < alpha)? XpR2 : xR2[s];
            
//            p[s+1] = dcat(x[s+1]);//distrNormald(x[s+1], x[s], 1.0/sigma2);  
            p[s+1] = h.getDensity(xR2[s+1]);

            //System.out.println("X:"+X+" Xp:"+Xp+" Px:"+Px+ "Pxp:"+Pxp+" alpha:"+alpha);
//            System.out.println("xR2["+s+"].array[0] = "+xR2[s].array[0]+", xR2["+s+"].array[1] = "+xR2[s].array[1]);
        }
/******************************************************************************/        
//        
        //graficacion MEZCLA DE GAUSIANAS EN R2         
        r = new HistogramR2(mhBN.x.length, 0.2f, -2, 8);
        r.Load(xR2);
//        r.Load(mhBN.x);
//        r.visible = false;
